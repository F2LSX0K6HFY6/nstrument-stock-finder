...
if st.button("Search"):
    ...
    if device.empty:
        ...
        instrument_range = str(device.iloc[0]["Instrument Range"]).strip()
        size = str(device.iloc[0]["Size"]).strip()
        ...
        matches = stock_df[
            (stock_df["Instrument Range"].astype(str).str.strip() == instrument_range)
            &
            (stock_df["Size"].astype(str).str.strip() == size)
        ]

        if matches.empty:
            st.warning("No matching stock items found")

        else:
            st.subheader("Matching Stock Codes")

            result = matches[["Code", "Description"]]
